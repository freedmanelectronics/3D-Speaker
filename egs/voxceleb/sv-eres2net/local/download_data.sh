...
download_dir=/mnt/work/jaesung/voxceleb
...
if [ ! -f ${download_dir}/vox2_aac.zip ]; then
  ...
  for part in a b c d e f g h; do
    wget --no-check-certificate --user vgg --password cookies https://thor.robots.ox.ac.uk/~vgg/data/voxceleb/vox1a/vox2_dev_aac_parta${part} -P ${download_dir}/voxceleb2 &
  done
  wait
