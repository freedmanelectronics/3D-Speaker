...
if [ "${stage}" -le 2 ] && [ "${stop_stage}" -ge 2 ]; then
  ...
  cat $video_list | while read video_file; do
    ...
    if [ ! -e $out_video_file ]; then
      ...
      ffmpeg -nostdin -y -i $video_file -qscale:v 2 -threads 16 -async 1 -r 25 $out_video_file
    fi
    if [ ! -e $out_wav_file ]; then
      echo "$(basename $0) Stage2: Extract audio from $filename"
      ffmpeg -nostdin -y -i $out_video_file -qscale:a 0 -ac 1 -vn -threads 16 -ar 16000 $out_wav_file
    fi
  done
fi
...
if [ ${stage} -le 6 ] && [ ${stop_stage} -ge 6 ]; then
  ...
  if [ -d "$exp/face_tracks" ]; then
    ...
    fill_gaps=$(grep "fill_gaps_in_merge:" $conf_file | awk '{print $2}')
    
    merge_args=""
    if [ "$merge_tracks" = "true" ]; then
      merge_args="--merge_tracks --max_gap_frames $max_gap_frames"
      
      # Add gap filling arguments if enabled
      if [ "$fill_gaps" = "true" ]; then
        merge_args="$merge_args --fill_gaps --raw_video_dir $raw_data_dir"
      fi
    fi
